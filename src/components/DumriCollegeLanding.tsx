...
export default function DumriCollegeLanding() {
  ...
  const [accrDesktopIndex, setAccrDesktopIndex] = useState(0);
    ...
    <div className="min-h-screen bg-white">
            ...
            return (
              <div className="max-w-6xl mx-auto">
                ...
                <div className="hidden md:block relative">
                  <motion.div
                    key={accrDesktopIndex}
                    initial={{ opacity: 0, x: 40 }}
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ duration: 0.4 }}
                    className="grid grid-cols-3 lg:grid-cols-5 gap-6 mb-8"
                  >
                  ...
                  <div className="flex items-center justify-center gap-4 mt-6">
                    <button
                      onClick={() =>
                        setAccrDesktopIndex((i) =>
                          Math.max(0, i - 1),
                        )
                      }
                      disabled={accrDesktopIndex === 0}
                      className="w-11 h-11 rounded-full bg-[#0C4D8B] disabled:bg-gray-300 text-white flex items-center justify-center shadow-lg hover:bg-[#084080] transition-all disabled:cursor-not-allowed text-xl font-bold"
                    ...
                    <div className="flex gap-2">
                      {[0, 1, 2].map((idx) => (
                        <button
                          key={idx}
                          onClick={() => setAccrDesktopIndex(idx)}
                          className={`h-2.5 rounded-full transition-all ${idx === accrDesktopIndex
                            ? "bg-[#0C4D8B] w-8"
              ...
              <>
                ...
                <div className="md:hidden">
                  <motion.div
                    ...
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ duration: 0.3 }}
                    onClick={() => setSelectedGalleryImage(galleryImages[galleryCarouselIndex])}
                    className="group cursor-pointer"
                  >
      ...
      {selectedGalleryImage && (
        ...
        >
          <button
            onClick={() => setSelectedGalleryImage(null)}
            className="absolute top-4 right-4 md:top-8 md:right-8 w-12 h-12 rounded-full bg-white/10 hover:bg-white/20 text-white flex items-center justify-center text-2xl font-bold transition-all z-10 backdrop-blur-sm"
