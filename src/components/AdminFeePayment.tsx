...
export function AdminFeePayment() {
  ...
  console.log('=== AdminFeePayment Component Mounted ===');
  console.log('Location state:', location.state);
  console.log('Student data received:', studentData);

  // Store student data in state to prevent loss on remount
  const [persistedStudentData, setPersistedStudentData] = useState<any>(null);

  // Set persisted data on first mount with valid data
  useEffect(() => {
    if (studentData && !persistedStudentData) {
      setPersistedStudentData(studentData);
    }
  }, [studentData]);

  // Use persisted data if available, otherwise use location state
  const currentStudentData = persistedStudentData || studentData;

  // Generate all 12 months with status based on pending months
  const allMonthsFees: MonthlyFee[] = currentStudentData ? [
    ...
    if (pendingMonth) {
      return {
        ...
        status: pendingMonth.daysOverdue > 0 ? 'overdue' as const : 'unpaid' as const,
        paidDate: undefined,
    ...
    // Months 1-2 are paid, rest are unpaid
    if (monthNumber <= 2) {
      return {
        month,
        monthNumber,
  ...
  const [fees] = useState<MonthlyFee[]>(allMonthsFees);
  const [selectedMonths, setSelectedMonths] = useState<number[]>([]);
  const [paymentMethod, setPaymentMethod] = useState<'card' | 'upi' | 'netbanking'>('upi');

  // Use useEffect to handle redirect - only if no data at all
  useEffect(() => {
    if (!currentStudentData && !studentData) {
      navigate('/admin/fee');
    }
  }, [currentStudentData, studentData, navigate]);

  // If no student data, return loading/null while redirect happens
  ...
  const handleMonthToggle = (monthNumber: number) => {
    const fee = fees.find(f => f.monthNumber === monthNumber);
  ...
  const handleMakePayment = () => {
    ...
    alert(`Processing payment of ₹${totalAmount.toLocaleString()} for ${currentStudentData.studentName}\n\nPayment Method: ${paymentMethod.toUpperCase()}\n\nSelected Months: ${selectedMonths.length}\n\nProcessed by Admin: Stevie Zone`);
    navigate('/admin/fee');
  };
      ...
      <div className="space-y-6 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Card className="p-6 bg-gradient-to-br from-green-50 to-emerald-50 border-2 border-green-200">
            ...
            <h2 className="text-3xl font-bold text-green-600">₹{(fees.filter(f => f.status === 'paid').length * 5000).toLocaleString()}</h2>
            <p className="text-xs text-slate-600 mt-2">{fees.filter(f => f.status === 'paid').length} months paid</p>
          </Card>
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <p className={`font-bold text-sm ${isPaid ? 'text-green-700' : 'text-slate-900'}`}>
                      {fee.month}
                    </p>
                    {!isPaid && (
                  ...
                  <div key={fee.monthNumber} className="pb-4 border-b border-amber-200 last:border-0">
                    ...
                    {feeBreakdown.map((item, index) => (
                      <div
                        ...
                        className="flex items-center justify-between py-2 px-3 hover:bg-amber-100 rounded"
                      >
