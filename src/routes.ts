...
const AddNewSubject = lazy(() =>
  import("./components/AddNewSubject").then((m) => ({
    default: m.AddNewSubject,
  }))
);
...
const ScheduleNewExam = lazy(() =>
  import("./components/ScheduleNewExam").then((m) => ({
    default: m.ScheduleNewExam,
  }))
);
const CreateNotice = lazy(() =>
  import("./components/CreateNotice").then((m) => ({
    default: m.CreateNotice,
  }))
);
